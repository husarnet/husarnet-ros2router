...
if [ "$(echo $husarnet_api_response | yq -r .result.is_ready)" == "true" ]; then
    ...
    if [ "$DS_ROLE" == "NONE" ]; then
        ...
        yq -i '.participants[1].listening-addresses[0].domain = strenv(HOST)' DDS_ROUTER_CONFIGURATION.yaml
        yq -i '.participants[1].connection-addresses[0].domain = strenv(HOST)' DDS_ROUTER_CONFIGURATION.yaml
